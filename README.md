...
http://developers.serendip.me/lib/sdk.js

Clients are encouraged to load the sdk asynchronously, either using JavaScript (see a similar example here https://developers.facebook.com/docs/reference/javascript/ , or using an async library such as requirejs).
...
Demo App
There’s a very simple demo app in demo/index.html, which provides an annotated script to use the JS SDK.
...
All SDK calls are asynchronous and adhere to the Deferred interface . Generally speaking, a call may either be resolved, or rejected.
...
The response will be returned as a generic object which in practive will hold the return data as defined in the Active Docs.
...
The complete API specification, along with the definition of the response objects is defined in the API Specification Docs 
...
For more details of how to use implicit login, see the demo app. 
...
Refer to API Specification Docs for more details
...
See Events for list of supported events
...
Events
auth.statusChange - fired whenever the login status of the user changes. Passes in a LoginObject object
